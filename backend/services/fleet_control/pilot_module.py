...
TASK_HANDOFF_DELIVERY = "handoff_delivery"
...
def run_pickup(robot: RobotManager, spec: dict) -> dict:
    """SDK-backed staging motion for pickup tasks near the source zone."""
    params = spec.get("params", {})
    source_zone = params.get("source_zone")
    approach_meters = float(params.get("approach_meters", 0.6))
    prep_ok = robot.stand_up() if robot.is_humanoid() else True
    ok = prep_ok and robot.walk_forward(approach_meters)
    return {
        "success": ok,
        "source_zone": source_zone,
        "approach_meters": approach_meters,
        "used_humanoid_staging": robot.is_humanoid(),
    }


def run_transport(robot: RobotManager, spec: dict) -> dict:
    """SDK-backed zone-to-zone transport using a simple route profile."""
    params = spec.get("params", {})
    source_zone = params.get("source_zone")
    destination_zone = params.get("destination_zone")
    path = _route_profile(source_zone, destination_zone)

    steps: list[dict] = []
    ok = True
    for command in path:
        if command["type"] == TASK_TURN:
            cmd_ok = robot.turn(float(command["params"]["degrees"]))
        else:
            cmd_ok = robot.walk_forward(float(command["params"]["meters"]))
        steps.append({"command": command, "success": cmd_ok})
        if not cmd_ok:
            ok = False
            break
...
def run_handoff_delivery(robot: RobotManager, spec: dict) -> dict:
    """SDK-backed short indoor handoff and placement path."""
    params = spec.get("params", {})
    destination_zone = params.get("destination_zone")
    ok = robot.walk_forward(float(params.get("handoff_meters", 0.8)))
    action_name = params.get("handoff_action", "release arm")
    action_ok = True
    if ok and robot.is_humanoid() and robot.supports_arm_actions():
        action_ok = robot.perform_action(str(action_name))
    return {
        "success": ok and action_ok,
        "destination_zone": destination_zone,
        "handoff_meters": float(params.get("handoff_meters", 0.8)),
        "handoff_action": action_name if robot.is_humanoid() else None,
        "used_arm_action": robot.is_humanoid() and robot.supports_arm_actions(),
    }
...
_TASK_RUNNERS = {
    ...
    TASK_PICKUP: run_pickup,
    TASK_TRANSPORT: run_transport,
    TASK_HANDOFF_DELIVERY: run_handoff_delivery,
}


def _route_profile(source_zone: str | None, destination_zone: str | None) -> list[dict]:
    """Small deterministic route profiles to bridge planned tasks into SDK motions."""
    normalized_source = (source_zone or "").lower()
    normalized_destination = (destination_zone or "").lower()
    if normalized_source == "dock" and normalized_destination == "entry":
        return [
            {"type": TASK_WALK_FORWARD, "params": {"meters": 1.6}},
            {"type": TASK_TURN, "params": {"degrees": 35}},
            {"type": TASK_WALK_FORWARD, "params": {"meters": 1.2}},
        ]
    if normalized_source == "entry" and normalized_destination == "kitchen":
        return [
            {"type": TASK_WALK_FORWARD, "params": {"meters": 1.0}},
            {"type": TASK_TURN, "params": {"degrees": -20}},
            {"type": TASK_WALK_FORWARD, "params": {"meters": 0.8}},
        ]
    if normalized_source == normalized_destination:
        return [{"type": TASK_WALK_FORWARD, "params": {"meters": 0.4}}]
    return [{"type": TASK_WALK_FORWARD, "params": {"meters": 1.0}}]
