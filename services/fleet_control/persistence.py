...
class ShadowWorldStore:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            ...
            self._ensure_column(conn, "mission_sessions", "request_json", "TEXT NOT NULL DEFAULT '{}'")
            self._ensure_column(conn, "mission_sessions", "proposal_json", "TEXT NOT NULL DEFAULT '{}'")
            self._ensure_column(conn, "mission_sessions", "validated_plan_json", "TEXT NOT NULL DEFAULT '{}'")
            self._ensure_column(conn, "mission_sessions", "approval_json", "TEXT NOT NULL DEFAULT '{}'")
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT OR REPLACE INTO mission_sessions (
                    ...
                    request_json, proposal_json, validated_plan_json, approval_json,
                ...
                (
                    ...
                    json.dumps(session.get("request", {})),
                    json.dumps(session.get("proposal", {})),
                    json.dumps(session.get("validated_plan", {})),
                    json.dumps(session.get("approval", {})),
    ...
    def get_mission_session(self, mission_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "proposal": json.loads(row[7]),
            "validated_plan": json.loads(row[8]),
            "approval": json.loads(row[9]),
            "task_ids": json.loads(row[10]),
