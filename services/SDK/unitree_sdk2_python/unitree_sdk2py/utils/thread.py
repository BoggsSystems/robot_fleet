...
# timerfd_create is Linux-only. On macOS we skip it and use the portable fallback.
try:
    from .timerfd import *
    _HAS_TIMERFD = True
...
class RecurrentThread(Thread):
    def __init__(self, interval: float = 1.0, target = None, name = None, args = (), kwargs = None):
        ...
        if interval is None or interval <= 0.0:
            ...
            # Portable fallback for macOS / Windows
            super().__init__(target=self.__LoopFunc_portable, name=name)
    ...
    def __LoopFunc_portable(self):
        """Portable timer loop using time.sleep — used on macOS where timerfd is unavailable."""
        while not self.__quit:
            ...
            sleep_time = self.__inter - elapsed
            if sleep_time > 0:
