  ...
  return (
    <Panel title="Twin Inspector" meta={asset.id}>
      <div className="detail-stack">
        ...
        <div className="detail-row"><span>Mission status</span><strong>{task?.missionStatus?.replace('_', ' ') ?? 'N/A'}</strong></div>
        <div className="detail-row"><span>Fallback robots</span><strong>{task?.fallbackRobotIds?.join(', ') ?? 'None'}</strong></div>
        <div className="detail-row"><span>Assignment reason</span><strong>{task?.assignmentRationale ?? 'N/A'}</strong></div>
        {task?.blockedReason ? <div className="detail-row"><span>Failure</span><strong>{task.blockedReason}</strong></div> : null}
      </div>
    </Panel>
  )
}
