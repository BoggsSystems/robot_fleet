          ...
          return (
            ...
            >
              <div className="task-row__header">
                <strong>
                  {task.stepIndex && task.totalSteps ? `Step ${task.stepIndex} / ${task.totalSteps}: ` : ''}
                  {task.summary}
                </strong>
              ...
              <div className="task-row__footer">
                ...
                {task.fallbackRobotIds?.length ? <span>Fallbacks {task.fallbackRobotIds.join(', ')}</span> : null}
                {task.missionStatus ? <span>Mission {task.missionStatus.replace('_', ' ')}</span> : null}
              </div>
              {task.blockedReason ? <div className="task-row__fault">Blocked: {task.blockedReason}</div> : null}
            </button>
          )
        })}
      </div>
